...
template <typename T> int diagGenMat( const Matrix<T>& A,
  ...
  Matrix<double>& righteigenvecs_real,Matrix<double>& righteigenvecs_imag) {

  // Check matrix is square
  plumed_assert( A.rw==A.cl );
  std::vector<double> da(A.sz);
  unsigned k=0;
  std::vector<double> evals_real(A.cl);
  std::vector<double> evals_imag(A.cl);
  // Transfer the matrix to the local array
  for (unsigned i=0; i<A.cl; ++i) for (unsigned j=0; j<A.rw; ++j) da[k++]=static_cast<double>( A(j,i) );

  int n=A.cl; int lwork=-1, info;
  int ldvl=2*n;
  int ldvr=2*n;
  std::vector<double> work(A.cl);
  //~ std::vector<int> isup(2*A.cl);
  std::vector<double> evecs_left(A.sz*2);
  std::vector<double> evecs_right(A.sz*2);
                       
  dgeev_("V","V",&n,da.data(),
                &n,evals_real.data(),evals_imag.data(),
                evecs_left.data(),&ldvl,evecs_right.data(),&ldvr,
  ...
  if( eigenvals_real.size()!=A.cl ) { eigenvals_real.resize( A.cl ); }
  if( eigenvals_imag.size()!=A.cl ) { eigenvals_imag.resize( A.cl ); }
  if( lefteigenvecs_real.rw!=A.rw || lefteigenvecs_real.cl!=A.cl ) { lefteigenvecs_real.resize( A.rw, A.cl ); }
  if( lefteigenvecs_imag.rw!=A.rw || lefteigenvecs_imag.cl!=A.cl ) { lefteigenvecs_imag.resize( A.rw, A.cl ); }
   if( righteigenvecs_real.rw!=A.rw || righteigenvecs_real.cl!=A.cl ) { righteigenvecs_real.resize( A.rw, A.cl ); }
  if( righteigenvecs_imag.rw!=A.rw || righteigenvecs_imag.cl!=A.cl ) { righteigenvecs_imag.resize( A.rw, A.cl ); }
  
  k=0;
  for(unsigned i=0; i<A.cl; ++i) {
    eigenvals_real[i]=evals_real[i];
    eigenvals_imag[i]=evals_imag[i];
    // N.B. For ease of producing projectors we store the eigenvectors
    // ROW-WISE in the eigenvectors matrix.  The first index is the
    // eigenvector number and the second the component
    for(unsigned j=0; j<A.rw; ++j){
	  unsigned id=k++;
	  lefteigenvecs_real(i,j)=evecs_left[id];
	  righteigenvecs_real(i,j)=evecs_right[id];
	}
    for(unsigned j=0; j<A.rw; ++j){
	  unsigned id=k++;
	  lefteigenvecs_imag(i,j)=evecs_left[id];
	  righteigenvecs_imag(i,j)=evecs_right[id];
	}
  }
